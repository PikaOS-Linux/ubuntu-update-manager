...
class HweSupportStatusTestCase(TestCase):
    ...
    INSTALLED_SUPPORTED_HWE_PKG_NAMES = ("xserver-xorg-core-lts-trusty",)
    INSTALLED_OTHER_PKG_NAMES = ("2vcard",)
    ...
    def setUp(self):
        self.cache = []
        INSTALL = (
            self.INSTALLED_UNSUPPORTED_HWE_PKG_NAMES
    ...
    def test_advice_about_hwe_status_is_running(self):
        with patch("hwe_support_status.is_unsupported_hwe_running") as m:
            with patch("builtins.print") as mock_print:
                ...
                mock_unsupported_hwe_pkgs.append(
                    make_mock_pkg(
                        self.INSTALLED_UNSUPPORTED_HWE_XORG_PKG_NAMES[0]
                    )
                )
                ...
                self.assertIn(
                    "Your current Hardware Enablement Stack (HWE) "
    ...
    def test_advice_about_hwe_status_installed_only(self):
        with patch("hwe_support_status.is_unsupported_hwe_running") as m:
            with patch("builtins.print") as mock_print:
                ...
                text = mock_print.call_args[0][0]
                self.assertIn(
                    "You have packages from the Hardware "
    ...
    def test_advice_about_hwe_status_supported_hwe_stack(self):
        with patch("hwe_support_status.is_unsupported_hwe_running") as m:
            with patch("builtins.print") as mock_print:
                ...
                mock_supported_hwe_pkgs.append(
                    make_mock_pkg(self.INSTALLED_SUPPORTED_HWE_PKG_NAMES[0])
                )
                from HweSupportStatus.consts import LTS_EOL_DATE
                from datetime import timedelta
                ...
                today = LTS_EOL_DATE - timedelta(14)
                hwe_support_status.advice_about_hwe_status(
                    [], mock_supported_hwe_pkgs, [], False, today, verbose=True
                )
                text = mock_print.call_args[0][0]
                self.assertIn(
