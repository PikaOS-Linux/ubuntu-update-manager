...
class UpdateGroup(UpdateItem):
    ...
    def selection_is_inconsistent(self):
        ...
        return len(pkgs_installing) > 0 and len(pkgs_installing) < len(
            self.items
        )
...
class UpdateApplicationGroup(UpdateGroup):
    def __init__(self, pkg, application, to_remove, sensitive=True):
        ...
        super(UpdateApplicationGroup, self).__init__(
            pkg, name, icon, to_remove, sensitive
        )
...
class UpdatePackageGroup(UpdateGroup):
    def __init__(self, pkg, to_remove, sensitive=True):
        name = utils.get_package_label(pkg)
        icon = Gio.ThemedIcon.new("package")
        super(UpdatePackageGroup, self).__init__(
            pkg, name, icon, to_remove, sensitive
        )
...
class UpdateSystemGroup(UpdateGroup):
    def __init__(self, cache, to_remove, sensitive=True):
        ...
        super(UpdateSystemGroup, self).__init__(
            None, name, icon, to_remove, sensitive
        )
...
class UpdateList:
    ...
    def __init__(self, parent, dist=None):
        ...
        self.application_dirs = [
            os.path.join(base, "applications") for base in data_dirs.split(":")
        ]
    ...
    def _get_application_for_package(self, pkg):
        ...
        for desktop_file in desktop_files:
            try:
                ...
                logging.warning(
                    "Error loading .desktop file %s: %s" % (desktop_file, e)
                )
                continue
    ...
    def _populate_desktop_cache(self, pkg_names):
        if not pkg_names:
            ...
            logging.warning(
                "_populate_desktop_cache called with empty list "
                "of packages."
            )
            return
        ...
        for desktop_file in glob.iglob(pattern):
            ...
            if pkg in pkg_names:
                ...
                logging.debug("App candidate for %s: %s" % (pkg, desktop_file))
    ...
    def _is_security_update(self, pkg):
        ...
        for ver in pkg._pkg.version_list:
            ...
            if (
                inst_ver
                and apt.apt_pkg.version_compare(ver.ver_str, inst_ver.ver_str)
                <= 0
            ):
                continue
            ...
            for (verFileIter, index) in ver.file_list:
                if (
                    verFileIter.archive == "%s-security" % self.dist
                    and verFileIter.origin == "Ubuntu"
                ):
    ...
    def _is_ignored_phased_update(self, pkg):
        ...
        if self.PHASED_UPDATES_KEY in pkg.candidate.record:
            ...
            if percentage > int(threshold):
                logging.info(
                    "holding back phased update %s (%s < %s)"
    ...
    def _make_groups(
        self, cache, pkgs, eventloop_callback, to_remove=False, sensitive=True
    ):
        if not pkgs:
            return []
        ungrouped_pkgs = []
        ...
        for pkg in pkgs:
            ...
            if app is not None:
                app_group = UpdateApplicationGroup(
                    pkg, app, to_remove, sensitive
                )
        ...
        if ungrouped_pkgs:
            ...
            linux_regexp = re.compile(
                "(" + "|".join(["^" + n for n in linux_names]) + ")"
            )
            ubuntu_base_group = UpdateGroup(
                None, None, None, to_remove, sensitive
            )
            ...
            for pkg in ungrouped_pkgs:
                if ubuntu_base_group.is_dependency(
                    pkg, cache, eventloop_callback
                ):
                    if system_group is None:
                        system_group = UpdateSystemGroup(
                            cache, to_remove, sensitive
                        )
                    system_group.add(pkg)
                else:
                    pkg_groups.append(
                        UpdatePackageGroup(pkg, to_remove, sensitive)
                    )
    ...
    def update(
        ...
        class FakeUbuntuProPackage:
            def __init__(self, package_name, version, size):
                ...
                self.candidate = FakeUbuntuProPackageCandidate(
                    package_name, version, size
                )
        ...
        for (package_name, version, size) in ua_security_packages:
            fake_ua_packages.append(
                FakeUbuntuProPackage(package_name, version, size)
            )
        ...
        for pkg in cache:
            if pkg.is_upgradable or pkg.marked_install:
                if getattr(pkg.candidate, "origins", None) is None:
                    ...
                    print(
                        "WARNING: upgradable but no candidate.origins?!?: ",
                        pkg.name,
                    )
                    continue
            ...
            if pkg.is_upgradable and not (
                pkg.marked_upgrade or pkg.marked_install
            ):
                self.held_back.append(pkg.name)
                continue
            if pkg.is_auto_removable and (
                cache.versioned_kernel_pkgs_regexp
                and cache.versioned_kernel_pkgs_regexp.match(pkg.name)
                and not cache.running_kernel_pkgs_regexp.match(pkg.name)
            ):
        ...
        if security_pkgs or upgrade_pkgs:
            ...
            pkg_names = [
                p.name
                for p in security_pkgs
                + upgrade_pkgs
                + kernel_autoremove_pkgs
                + duplicate_pkgs
            ]
            self._populate_desktop_cache(pkg_names)
        self.oem_groups = self._make_groups(
            cache, oem_pkgs, eventloop_callback
        )
        self.update_groups = self._make_groups(
            cache, upgrade_pkgs, eventloop_callback
        )
        self.security_groups = self._make_groups(
            cache, security_pkgs, eventloop_callback
        )
